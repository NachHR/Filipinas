...
function applyShellTranslations(){
  ...
  document.querySelector('#budgetDialog .dialog-actions button[value="cancel"]')?.replaceChildren(document.createTextNode(lang==='es'?'Cancelar':'Cancel'));
  document.getElementById('saveBudgetButton')?.replaceChildren(document.createTextNode(lang==='es'?'Guardar':'Save'));
}
window.addEventListener('load',()=>{
  applyShellTranslations();
  document.getElementById('languageToggle')?.addEventListener('click',()=>setTimeout(applyShellTranslations,0));
});
